...
void dfs_util(vector<vector<int> > &g, vector<int> &preOrder, vector<int> &postOrder, vector<int> &colors,  int &clock, int u) {
    clock++;
    preOrder[u] = clock;
    colors[u] = GRAY;
    ...
    for (int i = 0; i<g[u].size(); i++){
        int v = g[u][i];
        if (colors[v] == WHITE)
            dfs_util(g, preOrder, postOrder, colors, clock, v);
    }
    colors[u] = BLACK;
    clock++;
    postOrder[u] = clock;
...
void dfs(vector<vector<int> > &g){
    ...
    for (int i=0; i<n; i++){
        if (colors[i]==WHITE)
            dfs_util(g, preOrder, postOrder, colors, time, i);
    }
}
...
void dfs_visit_topo(vector<vector<int> > &g, vector<int> &colors, vector<int> &topoOrder, int u) {
    colors[u] = GRAY;
    for (int i = 0; i<g[u].size(); i++){
        int v = g[u][i];
        if (colors[v] == WHITE)
            dfs_visit_topo(g, colors, topoOrder, v);
    }
    colors[u] = BLACK;
    topoOrder.push_back(u);
}

vector<int> dfs_topo(vector<vector<int> > &g){
    int n = g.size();
    vector<int> colors(n, WHITE);
    ...
    for (int i=0; i<n; i++){
        if (colors[i]==WHITE)
            dfs_visit_topo(g, colors, topoOrder, i);
    }
